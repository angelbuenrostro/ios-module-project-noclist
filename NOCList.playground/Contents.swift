...
func findCleanAgents(agents: [(String, String, Int, Bool)]) -> [(String, String, Int, Bool)] {
...
print("\(findCleanAgents(agents: agents).count) clean agents out of \(agents.count) total agents.")
...
func findHighRisk() {
    ...
    for agent in agents {
        ...
        if agent.accessLevel >= 8 {
            ...
            var message = agent.realName + ", level: " + String(agent.accessLevel)
...
func levelCounts() {
    ...
    print("\(lowLevelAgents) low level agents, \(midLevelAgents) mid level agents, \(highLevelAgents) highLevelAgents")
...
func allAgents() {
    ...
    for agent in sortedAgents {
        ...
        print(agent.coverName + ", access level: " + String(agent.accessLevel))
